...
namespace G_Mobile_Android_WMS
{
    ...
    public class SettingsActivity : ActivityWithScanner
    {
        ...
        private void GetAndSetControls()
        {
            ...
            InnerSettings.BackgroundTintList = AppCompatResources.GetColorStateList(this, Resource.Color.floating_button_red);
            IdsSettings.BackgroundTintList = AppCompatResources.GetColorStateList(this, Resource.Color.button_blue);
            UserSettings.BackgroundTintList = AppCompatResources.GetColorStateList(this, Resource.Color.button_blue);
            Orientation.BackgroundTintList = AppCompatResources.GetColorStateList(this, Resource.Color.button_blue);
            ...
            Helpers.SetTextOnEditText(this, SettingsPassX, Globalne.CurrentTerminalSettings.Password);

            string OrientationStr = "";

            switch (Globalne.CurrentTerminalSettings.Orientation)
            {
                case Android.Content.PM.ScreenOrientation.Portrait: OrientationStr = this.GetString(Resource.String.orientation_vert); break;
                case Android.Content.PM.ScreenOrientation.Landscape: OrientationStr = this.GetString(Resource.String.orientation_horiz); break;
                case Android.Content.PM.ScreenOrientation.ReversePortrait: OrientationStr = this.GetString(Resource.String.orientation_revvert); break;
                case Android.Content.PM.ScreenOrientation.ReverseLandscape: OrientationStr = this.GetString(Resource.String.orientation_revhoriz); break;
                default: OrientationStr = this.GetString(Resource.String.orientation_vert); break;
            }
        ...
        private async void Orientation_Click(object sender, EventArgs e)
        {
            ...
            string Res = await UserDialogs.Instance.ActionSheetAsync(this.GetString(Resource.String.select_screen_orientation),
            ...
            {
                if (Res == Options[0])
                    Globalne.CurrentTerminalSettings.Orientation = Android.Content.PM.ScreenOrientation.Portrait;
                else if (Res == Options[1])
                    Globalne.CurrentTerminalSettings.Orientation = Android.Content.PM.ScreenOrientation.Landscape;
                else if (Res == Options[2])
                    Globalne.CurrentTerminalSettings.Orientation = Android.Content.PM.ScreenOrientation.ReversePortrait;
                else if (Res == Options[3])
                    Globalne.CurrentTerminalSettings.Orientation = Android.Content.PM.ScreenOrientation.ReverseLandscape;
                else
                    Globalne.CurrentTerminalSettings.Orientation = Android.Content.PM.ScreenOrientation.Portrait;
        ...
        private async Task SelectSettingsGroup()
        {
            try
            {
                int ID = await BusinessLogicHelpers.UserSettingGroups.ShowSelectListOfUserGroups(this, Resource.String.settings_activity_selectsetgroup, true);
        ...
        private async void InnerSettings_Click(object sender, EventArgs e)
        {
            try
            {
                PromptResult Res = await Helpers.AlertAsyncWithPrompt(this, Resource.String.settings_getpass, null, "", InputType.Password);

                if (!Res.Ok)
                    return;

                if (!Globalne.operatorBL.SprawdźHasłoOperatora(Int32.MaxValue, Encryption.RSAEncrypt(Res.Text)))
                {
                    Helpers.PlaySound(this, Resource.Raw.sound_alert);
                    return;
                }

                RunIsBusyAction(() => Helpers.SwitchAndFinishCurrentActivity(this, typeof(InnerSettingsActivity)));
            }
            catch (Exception ex)
            {
                Helpers.HandleError(this, ex);
        ...
        public void SaveSettingsAndConnect()
        {
            ...
            {
                RunOnUiThread(() => Helpers.CenteredToast(GetString(Resource.String.settings_activity_connecterror), ToastLength.Short));
            }
            else
            {
                ...
                if (Res == 0)
                {
                    ...
                    RunOnUiThread(() => Helpers.CenteredToast(GetString(Resource.String.settings_activity_saved), ToastLength.Short));
                }
                else
                    RunOnUiThread(() => Helpers.CenteredToast(GetString(Resource.String.settings_activity_connecterror), ToastLength.Short));
            }
